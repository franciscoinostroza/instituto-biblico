...
class AcademicoSeeder extends Seeder
{
    ...
    {
        $teologia = Carrera::firstOrCreate(
        ...
        $ministerio = Carrera::firstOrCreate(
        ...
        $periodo = PeriodoLectivo::firstOrCreate(
        ...
        $hermeneutica = Materia::firstOrCreate(
            ['code' => 'TEO-101'],
            [
        ...
        $homiletica = Materia::where('code', 'MIN-101')->first();

        $at = Materia::firstOrCreate(
            ['code' => 'TEO-102'],
            [
        ...
        // Inscripciones (idempotentes por par materia+estudiante)
        foreach ([$est1, $est2, $est3] as $est) {
            Inscripcion::firstOrCreate(
                ['materia_id' => $hermeneutica->id, 'estudiante_id' => $est->id],
        ...
        foreach ([$est1, $est2] as $est) {
            Inscripcion::firstOrCreate(
                ['materia_id' => $homiletica->id, 'estudiante_id' => $est->id],
                ['fecha_inscripcion' => now(), 'active' => true]
            );
        }

        Inscripcion::firstOrCreate(
            ['materia_id' => $at->id, 'estudiante_id' => $est3->id],
            ['fecha_inscripcion' => now(), 'active' => true]
        );

        PlanDeCurso::firstOrCreate(
        ...
        Anuncio::firstOrCreate(
            ['materia_id' => $hermeneutica->id, 'title' => 'Bienvenidos a Hermenéutica Bíblica'],
            [
                'autor_id'     => $docente1->id,
                'body'         => "Estimados estudiantes:\n\nBienvenidos a esta materia fundamental para su formación teológica. Les pido que lean el capítulo 1 del libro de Fee y Stuart antes de nuestra primera clase.\n\nProf. Juan Pérez",
                'published_at' => now()->subDays(2),
            ]
        );

        Anuncio::firstOrCreate(
            ['materia_id' => $hermeneutica->id, 'title' => 'Material del primer parcial disponible'],
            [
                'autor_id'     => $docente1->id,
        ...
        $tarea = Tarea::firstOrCreate(
            ['materia_id' => $hermeneutica->id, 'title' => 'Análisis exegético — Salmo 23'],
            [
        ...
        $examen = Examen::firstOrCreate(
            ['materia_id' => $hermeneutica->id, 'title' => 'Control de Lectura — Fee & Stuart Cap. 1-3'],
            [
        ...
        // Preguntas del examen (solo si es nuevo)
